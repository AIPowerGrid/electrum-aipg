...
PACKAGE=Electrum-AIPG
GIT_REPO=https://github.com/AIPowerGrid/electrum-aipg
...
info "Installing dependencies sepcific to aipg..."
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-aipg-pypi.txt \
    || fail "Could not install dependencies from pypi"
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-aipg-git.txt \
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-aipg-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-aipg-${VERSION}.dmg"
