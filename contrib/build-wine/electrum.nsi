...
;Variables
  ...
  !define PRODUCT_NAME "Electrum AIPG"
  !define PRODUCT_WEB_SITE "https://github.com/AIPowerGrid/electrum-aipg"
  !define PRODUCT_PUBLISHER "The AIPG Community"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-aipg-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe" 0
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-aipg-${PRODUCT_VERSION}.exe$\" $\"%1$\""
