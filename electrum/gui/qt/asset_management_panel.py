...
class ManageAssetPanel(QWidget, Logger):
    def __init__(self, parent: 'AssetTab'):
        ...
        self.send_button = EnterButton(_("Pay") + f" {self.burn_amount} AIPG...", self._create_tx)
...
class CreateAssetPanel(ManageAssetPanel):
    ...
    def _clayout_on_edit(self, clayout: ChoicesLayout):
        ...
        self.send_button.setText(_("Pay") + f" {self.burn_amount} AIPG...")
    ...
    def _create_tx(self):
        ...
        def make_tx(fee_est, *, confirmed_only=False):
            ...
            if asset_type == AssetType.RESTRICTED:
                # https://github.com/AIPowerGrid/AI-Power-Grid-Core/blob/master/src/assets/assets.cpp#L4571
                # https://github.com/AIPowerGrid/AI-Power-Grid-Core/blob/master/src/assets/assets.cpp#L901
            ...
            try:
                if not goto_address:
                    # Freeze a change address so it is seperate from the AIPG change
                    self.parent.wallet.set_reserved_state_of_address(asset_change_address, reserved=True)
